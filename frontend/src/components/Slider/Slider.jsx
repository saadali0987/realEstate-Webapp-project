    ...
    <div className='h-[350px] w-full flex gap-[20px]'>
      {sliderImageIndex != null && (<div className='fullSlider z-10 absolute w-screen h-screen top-0 left-0 bg-black flex items-center justify-between'>
        ...
        <div className='imgContainer flex-[10]'>
          <img src={images[sliderImageIndex].url} className='w-full h-full object-cover' alt="" />
        </div>
      ...
      <div className='bigImage flex-[3]'>
        <img className='w-full h-full rounded-md object-cover cursor-pointer ' src={images[0].url} alt="" onClick={()=>setSliderImageIndex(0)} />
      </div>
      <div className='smallImages flex-[1] flex flex-col justify-between gap-[20px]'>
        {images.slice(1).map((image, index)=>(<img className='h-[100px] w-full rounded-md object-cover cursor-pointer' onClick={()=>setSliderImageIndex(index + 1)} src={image.url} key={index} />))}
      </div>
    </div>
  )
}
