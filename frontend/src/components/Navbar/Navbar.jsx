...
const Navbar = () => {
  ...
  return (
    <nav className='flex justify-between h-[100px] items-center'>
        ...
        <div className='flex-[2]  flex items-center justify-end h-full lg:bg-gray-200'>
            <a className=' hidden md:inline hover:scale-[1.05] ease-in duration-100 py-[12px] px-[24px] m-[20px]' href="">Sign Up</a>
            <a className=' hidden md:inline py-[12px] px-[24px] m-[20px] rounded-md bg-yellow-400  hover:scale-[1.05] ease-in duration-100 ' href="">Sign In</a>
            <div>
                <img className={`md:hidden w-[36px] h-[36px] sticky cursor-pointer z-[99]`} onClick={()=>setShowMenu(prev=>!prev)} src={menu} alt="menu" />
            </div>
            <div className={`bg-black text-white absolute ease duration-1000 flex flex-col items-center justify-center w-[50%] gap-[50px] size-[28px]  h-screen top-0 ${showMenu ? "right-0" : "right-[-50%]"}`}>
